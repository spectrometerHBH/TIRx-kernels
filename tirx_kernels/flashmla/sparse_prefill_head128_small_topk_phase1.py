        ...
        def perform_o_copy_out(o_s_q_idx, o_outer_loop_phase, is_last_o: T.constexpr):
            ...
            for epi_k in T.unroll((D_V // 2) // B_EPI):
                ...
                for o_i in T.unroll(B_EPI // 8):
                    ...
                    T.ptx.st(
                        q_smem.view("uint32").ptr_to([idx_in_warpgroup % 64, q_smem_col // 2]),
                        o_epi_bf16[0],
                        o_epi_bf16[1],
                        o_epi_bf16[2],
                        o_epi_bf16[3],
        ...
        while wg3_job_valid != 0:
            ...
            for k in T.serial(0, wg3_num_k_blocks, unroll=False):
                ...
                if local_warp_idx < 2:
                    Tx.wg.copy_async(
                        p_frag[:, :],
                        tmem_ldst[:, TMEM_COL_P : TMEM_COL_P + WG3_NUM_ELEMS_PER_THREAD],
                    )
                    Tx.wg.copy_async(
                        p_peer_frag[:, :],
                        tmem_ldst[
                            :,
                            TMEM_COL_P + WG3_NUM_ELEMS_PER_THREAD : TMEM_COL_P

...
def _init_pipeline(smem_raw, full_off, empty_off, stages, producers, consumers):
    for stage in range(stages):
        T.ptx.mbarrier.init(smem_raw.ptr_to([full_off + stage * 8]), producers)
        T.ptx.mbarrier.init(smem_raw.ptr_to([empty_off + stage * 8]), consumers)
...
def _producer_acquire(smem_base_addr, empty_off, count, stages):
    return T.ptx.mbarrier.try_wait(
...
def _software_commit(smem_base_addr, full_off, count, stages):
    return T.ptx.mbarrier.arrive(_pipe_full_addr(smem_base_addr, full_off, count, stages))


def _consumer_release(smem_base_addr, empty_off, count, stages):
    return T.ptx.mbarrier.arrive(_pipe_empty_addr(smem_base_addr, empty_off, count, stages))
...
def _consumer_wait_state(smem_base_addr, full_off, index, phase):
    return T.ptx.mbarrier.try_wait(_shared_addr(smem_base_addr, full_off + index * 8), phase)


def _producer_acquire_state(smem_base_addr, empty_off, index, phase):
    return T.ptx.mbarrier.try_wait(_shared_addr(smem_base_addr, empty_off + index * 8), phase)


def _software_commit_state(smem_base_addr, full_off, index):
    return T.ptx.mbarrier.arrive(_shared_addr(smem_base_addr, full_off + index * 8))


def _consumer_release_state(smem_base_addr, empty_off, index):
    return T.ptx.mbarrier.arrive(_shared_addr(smem_base_addr, empty_off + index * 8))
...
def _mma_commit(barrier):
    return T.ptx.tcgen05.commit(barrier, cta_group=1, pred=T.ptx.elect_sync())
...
def _ld_shared_v4_u32(ptr, values):
    return T.ptx.ld(
        ...
        "b32",
        space="shared",
        dst=tuple(T.address_of(values[i]) for i in range(4)),
        vec="v4",
    )
...
def _pack_f16x2(a, b):
    ...
    return T.ptx.cvt("f16x2", b, a, atype="f32", rounding="rn")
...
def _mma_m16n8k16_f16_zero(acc, a, b, acc_off, b_off):
    return T.ptx.mma(
        ...
        "float16",
        "float32",
        [T.address_of(acc[acc_off + i]) for i in range(4)],
        [T.address_of(a[i]) for i in range(4)],
        [T.address_of(b[b_off + i]) for i in range(2)],
    )
...
def _cg0_tmem_ld(tmem_base, stage, thread, values):
    ...
    T.ptx.tcgen05.ld(addr, *[values[i] for i in range(16)], shape="16x32bx2", num=16)
    T.ptx.tcgen05.ld(addr + 32, *[values[16 + i] for i in range(16)], shape="16x32bx2", num=16)
...
def _invert_diagonal_8x8(smem_raw, stage, block8, lane):
    ...
    for src_row in T.unroll(7):
        ...
        row_scale: T.float32 = T.ptx.neg_f32(row_values[src_row])
        for i in T.unroll(7):
            if i < src_row:
...
def _inverse_8_to_16(smem_raw, stage, block16, lane):
    ...
    T.ptx.ldmatrix(
        ...
        1,
        ".b16",
        _f16_tile_ptr(smem_raw, SMEM_AINV_OFF, stage, block16 + (lane & 7), block16),
        T.address_of(b[0]),
    )
    _mma_m16n8k8_f16_zero(acc, a, b)
...
def _ldmatrix_x4_tile(smem_raw, stage, base_row, base_col, lane, transpose, dst):
    ...
    T.ptx.ldmatrix(
        ...
        4,
        ".b16",
        _f16_tile_ptr(smem_raw, SMEM_AINV_OFF, stage, row, col),
        *[T.address_of(dst[i]) for i in range(4)],
    )
...
def _stmatrix_x4_tile(smem_raw, stage, base_row, base_col, lane, src):
    ...
    T.ptx.stmatrix(
        ...
        ".b16",
        _f16_tile_ptr(smem_raw, SMEM_AINV_OFF, stage, row, col),
        *[T.address_of(src[i]) for i in range(4)],
    )
...
def _inverse_32_to_64(smem_raw, stage, local_warp, lane):
    ...
    T.ptx.bar.sync(INVERSE_BARRIER, 128)
...
def _state_tmem_ld_sub(tmem_base, thread, sub, values, value_offset):
    ...
    T.ptx.tcgen05.ld(addr, *[values[value_offset + i] for i in range(32)], shape="32x32b", num=32)
...
def _state_tmem_st_sub(tmem_base, thread, sub, values, value_offset):
    ...
    addr: T.int32 = tmem_base + _cg1_tmem_row_bits(thread) + sub * 32
    T.ptx.tcgen05.st(addr, *[values[value_offset + i] for i in range(32)], shape="32x32b", num=32)
...
def _state_input_tmem_st_sub(tmem_base, thread, sub, values):
    ...
    T.ptx.tcgen05.st(addr, *[values[sub * 16 + i] for i in range(16)], shape="32x32b", num=16)
...
def _cg1_tmem_ld_f32(tmem_base, column, thread, values):
    ...
    T.ptx.tcgen05.ld(addr, *[values[i] for i in range(32)], shape="16x256b", num=8)
    T.ptx.tcgen05.ld(
        addr + T.int32(0x100000), *[values[32 + i] for i in range(32)], shape="16x256b", num=8
    )
...
def _cg1_tmem_st_f32(tmem_base, column, thread, values):
    ...
    addr: T.int32 = tmem_base + _cg1_tmem_row_bits(thread) + column
    T.ptx.tcgen05.st(addr, *[values[i] for i in range(32)], shape="16x256b", num=8)
    T.ptx.tcgen05.st(
        addr + T.int32(0x100000), *[values[32 + i] for i in range(32)], shape="16x256b", num=8
    )
...
def _load_v_fragment(smem_raw, stage, thread, values):
    ...
    for band in range(4):
        T.ptx.ldmatrix(
            ...
            *[T.address_of(values[band * 4 + i]) for i in range(4)],
        )
    ...
    for band in range(4):
        T.ptx.ldmatrix(
            ...
            ".b16",
            smem_raw.ptr_to([SMEM_V_OFF + stage_byte + lane_byte + band * 2048]),
            *[T.address_of(values[16 + band * 4 + i]) for i in range(4)],
        )
...
def _load_initial_state(
    ...
    for sub in range(4):
        for vector in range(8):
            T.ptx.ld(
                ...
                "b32",
                dst=(
                    T.address_of(state_sub[vector * 4]),
                    T.address_of(state_sub[vector * 4 + 1]),
                    T.address_of(state_sub[vector * 4 + 2]),
                    T.address_of(state_sub[vector * 4 + 3]),
                ),
                space="global",
                vec="v4",
    ...
    T.ptx.bar.sync(INITIAL_STATE_BARRIER, 128)
    ...
    for kphase in range(8):
        ...
        T.evaluate(
            T.ptx.tcgen05.mma(
                ...
                use_a_tmem=False,
                cta_group=1,
                enable_input_d=0 if kphase == 0 else 1,
                pred=T.ptx.elect_sync(),
            )
        )
    ...
    for kphase in range(4):
        ...
        T.evaluate(
            T.ptx.tcgen05.mma(
                ...
                use_a_tmem=True,
                cta_group=1,
                enable_input_d=accumulate if kphase == 0 else 1,
                pred=T.ptx.elect_sync(),
            )
        )
...
def _tma_qk_g2s(smem_base_addr, smem_off, descriptor, full_barrier, token, qk_head):
    ...
    for d_coord in range(0, 128, 64):
        T.ptx.cp_async.bulk.tensor.g2s_cta(
            ...
            full_barrier,
            T.reinterpret("uint64", descriptor),
            1,
            "",
            d_coord,
            T.cast(token, "int32"),
            qk_head,
            cache_policy=T.uint64(0),
...
def _load_qkv_chunk(
    ...
    *,
    ...
    if T.ptx.elect_sync():
        T.ptx.mbarrier.arrive.expect_tx(k_full, 16384)
        _tma_qk_g2s(
            smem_base_addr,
    ...
    if T.ptx.elect_sync():
        T.ptx.mbarrier.arrive.expect_tx(q_full, 16384)
        _tma_qk_g2s(
            smem_base_addr,
    ...
    if T.ptx.elect_sync():
        T.ptx.mbarrier.arrive.expect_tx(v_full, 16384)
        _tma_v_g2s(
            smem_base_addr,
...
def _load_gate_beta_chunk(
    ...
    *,
    ...
    cumprod0: T.float32 = T.ptx.exp2(gate0)
    cumprod1: T.float32 = T.ptx.exp2(gate1)
    ...
    T.ptx.cp_async.mbarrier.arrive.noinc(_shared_addr(smem_base_addr, 224 + beta_count * 8))
        ...
        for d_coord in range(0, 128, 64):
            if HQ == HV:
                T.ptx.cp_async.bulk.tensor.s2g(
                    ...
                    T.reinterpret("uint64", descriptor_o),
                    "",
                    d_coord,
...
def _kernel(
    ...
    *,
    ...
    if warp == 8:
        T.evaluate(T.ptx.prefetch_tensormap(T.address_of(q_map)))
        T.evaluate(T.ptx.prefetch_tensormap(T.address_of(k_map)))
        T.evaluate(T.ptx.prefetch_tensormap(T.address_of(v_map)))
        T.evaluate(T.ptx.prefetch_tensormap(T.address_of(o_map)))
    ...
    if warp <= 3:
        ...
        T.ptx.bar.sync(TMEM_ALLOC_BARRIER, 320)
        tmem_base_cg0: T.int32 = T.ptx.ld_volatile(
            T.address_of(tmem_holding[0]), "int32", "s32", space="shared"
        )
        ...
        while work_linear_cg0 < total_work:
            ...
            for pair_cg0 in T.serial(0, num_pairs_cg0):
                ...
                for frag_idx_cg0 in T.unroll(32):
                    ...
                    if frag_idx_cg0 >= 16:
                        col_cg0 = col_cg0 + 16
                    transfer0_cg0[frag_idx_cg0] = T.Select(
                        row_cg0 >= col_cg0,
                        T.ptx.exp2(row_log0_cg0 - s_cumsumlog[gate0_stage_cg0 * 64 + col_cg0]),
                        T.float32(0.0),
                    )
                    transfer1_cg0[frag_idx_cg0] = T.Select(
                        row_cg0 >= col_cg0,
                        T.ptx.exp2(row_log1_cg0 - s_cumsumlog[gate1_stage_cg0 * 64 + col_cg0]),
                        T.float32(0.0),
                    )
                ...
                for pair_frag_cg0 in T.unroll(16):
                    packed_mul_cg0 = T.ptx.mul_f32x2(
                        T.cuda.make_float2(
                ...
                for qk_in_pair_cg0 in T.unroll(2):
                    ...
                    for pair_frag_cg0 in T.unroll(16):
                        ...
                        qk_mul_cg0 = T.ptx.mul_f32x2(
                            qk_mul_cg0,
                            T.cuda.make_float2(scale, scale),
                            rounding="rn",
                            ftz=False,
                            dps=False,
                        )
    ...
    if warp >= 4 and warp <= 7:
        ...
        T.ptx.setmaxnreg(True, 256)
        if warp == 4:
            T.ptx.tcgen05.alloc(T.address_of(tmem_holding[0]), n_cols=TMEM_COLUMNS, cta_group=1)
        T.ptx.bar.sync(TMEM_ALLOC_BARRIER, 320)
        tmem_base_cg1: T.int32 = T.ptx.ld_volatile(
            T.address_of(tmem_holding[0]), "int32", "s32", space="shared"
        )
            ...
            if padded_chunks_cg1 > 0:
                ...
                for chunk_cg1 in T.serial(0, padded_chunks_cg1):
                    ...
                    for factor_group_cg1 in T.unroll(8):
                        ...
                        decay_factor_cg1[factor_group_cg1 * 2] = T.ptx.exp2(
                            T.cuda.float2_x(decay_diff_cg1[0])
                        )
                        decay_factor_cg1[factor_group_cg1 * 2 + 1] = T.ptx.exp2(
                    ...
                    for pair_cg1 in T.unroll(32):
                        ...
                        v_words_cg1[pair_cg1] = T.ptx.sub_f16x2(v_words_cg1[pair_cg1], ks_word_cg1)
        ...
        if warp == 4:
            ...
            T.ptx.tcgen05.dealloc(tmem_base_cg1, TMEM_COLUMNS, cta_group=1)
        ...
        T.ptx.bar.sync(TMEM_ALLOC_BARRIER, 320)
        tmem_base_issuer0: T.int32 = T.ptx.ld_volatile(
            T.address_of(tmem_holding[0]), "int32", "s32", space="shared"
        )
        ...
        T.ptx.setmaxnreg(False, 24)
        T.ptx.bar.sync(TMEM_ALLOC_BARRIER, 320)
        tmem_base_issuer1: T.int32 = T.ptx.ld_volatile(
            T.address_of(tmem_holding[0]), "int32", "s32", space="shared"
        )

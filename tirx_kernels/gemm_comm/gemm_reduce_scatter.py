...
CONFIGS = [
    {
        ...
        "dtype": "float16",
        "label": f"tp{WORLD_SIZE}_m{M}_n{N}_k{TOTAL_K}_fp16",
    }
]


def _check_config(M_: int, N_: int, K_: int, world_size: int, dtype: str) -> None:
    expected = (M, N, TOTAL_K, WORLD_SIZE, "float16")
...
class _Case:
    ...
    def launch(self) -> None:
        ...
        tvm.get_global_func("runtime.disco.transfer_to_peers_reduce_scatter")(
            self.semaphore,
            self.gemm_out,
            self.staging,
        ...
        barrier_on_communication_stream(self.runtime)
...
def _allocate_case(
    ...
    gemm_out = symmetric_empty(runtime, (M, N), d_type)
    ...
    case = _Case(
        ...
        gemm_out=gemm_out,
        semaphore=semaphore,
        staging=symmetric_empty(runtime, (WORLD_SIZE, LOCAL_M, N), d_type),
        out=torch.empty((LOCAL_M, N), dtype=torch.float16, device=device),
...
def _check_correctness(case: _Case) -> None:
    ...
    begin = case.runtime.rank * LOCAL_M
    end = begin + LOCAL_M
...
def run_test(
    ...
    **_kwargs: Any,
    ...
    _check_config(M, N, K, world_size, dtype)
    run_distributed(
        get_kernel(M, N, K, world_size, dtype),
        world_size=world_size,
        worker=_run_worker,
        mode="test",
        worker_kwargs={
...
def run_bench(
    ...
    **_kwargs: Any,
    ...
    """Pair TIRx and distributed GEMM baselines under the shared Kineto timer."""

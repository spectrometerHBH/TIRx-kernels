...
def _check_gemm_comm_libraries(workloads: list[dict]) -> None:
    """Fail fast when GemmComm's pip-provided libraries are not discoverable.

    GemmComm workloads lock and preload the pip-installed NCCL/cuBLAS/
    cuBLASMp/NVSHMEM libraries so a loader-path change cannot silently alter
    the comparison. Discovery is deterministic per environment, so bench
    subprocesses resolve the same files on their own; here we only validate
    that resolution succeeds and log what will be used.
    """
    if not _uses_gemm_comm(workloads):
        return
    from tirx_kernels.gemm_comm._lib_discovery import discover_libraries, ensure_nvshmem_home

    try:
        libraries = discover_libraries()
        nvshmem_home = ensure_nvshmem_home()
    except RuntimeError as error:
        print(f"[bench-suite] gemm_comm library check failed: {error}", file=sys.stderr)
        sys.exit(2)
    print(f"[bench-suite] gemm_comm libraries: NVSHMEM_HOME={nvshmem_home}")
    for name, path in sorted(libraries.items()):
        print(f"[bench-suite]   {name}: {path}")
...
def main() -> None:
    ...
    # Fail fast when GemmComm's pip-provided communication libraries are not
    # discoverable, before any GPU work; discovery failure would otherwise
    # surface mid-sweep inside a spawned rank worker.
    _check_gemm_comm_libraries(workloads)

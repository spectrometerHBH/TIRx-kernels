...
fn execute_gmem_atomic_add<'a, 'k>(
    ...
    // Serialized RMW (the interpreter applies one CTA's atomic at a time). Run in
    // both modes so the trace's release event carries the same post-increment
    // counter the value run produces.
...
fn execute_gmem_wait_eq<'a, 'k>(
    ...
    if current != want {
        ...
        // rendezvous machinery). Another CTA's atomic_add will bump the counter;
        // if the awaited value is never produced, no progress in a full round ->
        // deadlock (the faithful behavior for a wrong lock-value).

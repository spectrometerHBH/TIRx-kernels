...
def build_gdn_prefill(config: GdnPrefillConfig = GdnPrefillConfig()) -> Kernel:
    ...
    # state S (f32), state_inp (fp16 undecayed-S copy for GEMM3/4), q_state acc
    ...
    state_inp = TmemBand(col0=128, dtype=iod)  # (128, V_DIM) fp16: 128-191 (64 cells)
    ...
    shared_inp = TmemBand(col0=448, dtype=iod)  # (V_DIM, BT) fp16, stage A: delta(G5)→NV(G6)
    shared_inp_b = TmemBand(col0=480, dtype=iod)  # (V_DIM, BT) fp16, stage B: vnew_gated(G7)
...
def _emit(k, config, n_chunks, sched, task_geom, args, sm, tm, rg, bars):
    ...
    def issue(
        ...
        # a_row0/b_row0 = base row offset into the operand (for the K double-buffer
        # stage = (c%2)*BT, which is compile-time for fixed-length and runtime for varlen).
        # TMEM operands (state_inp / shared_inp) are flat physical addresses: the
        # per-issue element offsets fold into the band's `elem` address.
        for g in range(kk // 16):
            a_off, a_sh = (
                ((a_row0 + g * 16, 0), (16, m)) if trans_a else ((a_row0, g * 16), (m, 16))
            )
            b_off, b_sh = (
                ((b_row0 + g * 16, 0), (16, n)) if trans_b else ((b_row0, g * 16), (n, 16))
            )
            a_op = a.elem(*a_off) if isinstance(a, TmemBand) else _sl(a, a_off, a_sh)
            b_op = b.elem(*b_off) if isinstance(b, TmemBand) else _sl(b, b_off, b_sh)
            k.tcgen05_mma(
                dst.at(0, 0),
                a_op,
                b_op,

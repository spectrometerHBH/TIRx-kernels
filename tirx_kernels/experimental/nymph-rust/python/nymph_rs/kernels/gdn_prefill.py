...
CONFIGS = [
    ...
    for ns, T in [
        (1, 64), (1, 128), (1, 192), (1, 256), (1, 512), (1, 1024), (1, 2048),
        (2, 128), (2, 256), (2, 512), (4, 128), (4, 256), (8, 64),
        (20, 64), (20, 192), (32, 128), (48, 64),   # num_work > SM_COUNT: multi-tile/CTA
    ]
]
...
CONFIGS_SUPPORTED = [
    {"num_seqs": ns, "seqlen": T, "label": f"ns{ns}_t{T}"}
    for ns, T in [(1, 64), (1, 128), (1, 192), (1, 256), (2, 128), (2, 256), (4, 128),
                  (20, 64), (20, 192)]
]
...
VARLEN_CONFIGS = [
    ...
    for s in [
        [64, 128], [128, 64, 192], [64, 64], [70, 130], [6, 100], [200],
        [13, 64, 191], [256, 320], [64, 64, 64, 64], [320, 7, 100],
    ]
] + [
    # multi-tile/CTA varlen (num_work = num_seqs·HV > SM_COUNT): mixed + non-BT-multiple.
    {"seqlens": [70, 64, 130, 6, 192, 100, 64, 128, 7, 200,
                 64, 191, 64, 256, 13, 64, 128, 320, 100, 70], "label": "v_mt20"},
]
...
def build_gdn_prefill(config: GdnPrefillConfig = GdnPrefillConfig()) -> Kernel:
    ...
    sizes = dict(
        k=2 * BT * K_DIM * 2, q=BT * K_DIM * 2, v=BT * V_DIM * 2,  # k double-buffered (2 stages)
        vnewt=V_DIM * BT * 2,
        attn=BT * BT * 2, tmpt=V_DIM * BT * 2,
        out=BT * V_DIM * 2, m=BT * BT * 4, gcs=BT * 4, beta=BT * 4,
    ...
    k = IRBuilder("nymph_gdn_prefill", num_warps=NUM_WARPS,
                  smem_size_bytes=off, launch_shape=launch_shape)
    ...
    cu_g = k.arg(space=MemorySpace.GMEM, dtype=DType.I32, shape=(NS + 1,)) if config.varlen else None
    ...
    def task_geom(task):
        ...
        work = task.field("work"); seq = work // NEFF; eh = work % NEFF
        shared = eh // HR
    ...
    tmem_base = k.tensor(space=MemorySpace.TMEM, dtype=DType.F32, shape=(128, N_COLS_TMEM),
                         layout=TmemLayout(TmemLayoutKind.LANE_128, col_start=0))
    s_tmem = k.tensor(space=MemorySpace.TMEM, dtype=DType.F32, shape=(128, V_DIM),
                      layout=TmemLayout(TmemLayoutKind.LANE_128, col_start=0))           # 0-127
    state_inp = k.tensor(space=MemorySpace.TMEM, dtype=iod, shape=(128, V_DIM),
                         layout=TmemLayout(TmemLayoutKind.LANE_128, col_start=128))       # 128-191 (64 cols bf16)
    qstate_tmem = k.tensor(space=MemorySpace.TMEM, dtype=DType.F32, shape=(64, V_DIM),
                           layout=TmemLayout(TmemLayoutKind.LANE_64_UPPER, col_start=192))  # 192-319
    ...
    acc_s0 = k.tensor(space=MemorySpace.TMEM, dtype=DType.F32, shape=(64, BT),
                      layout=TmemLayout(TmemLayoutKind.LANE_64_UPPER, col_start=320))      # stage 0: 320-383
    acc_s1 = k.tensor(space=MemorySpace.TMEM, dtype=DType.F32, shape=(64, BT),
                      layout=TmemLayout(TmemLayoutKind.LANE_64_UPPER, col_start=384))      # stage 1: 384-447
    acc_tmem = k.tensor(space=MemorySpace.TMEM, dtype=DType.F32, shape=(64, V_DIM),
                        layout=TmemLayout(TmemLayoutKind.LANE_64_UPPER, col_start=320))    # union view (ks/nv)
    shared_inp = k.tensor(space=MemorySpace.TMEM, dtype=iod, shape=(V_DIM, BT),
                          layout=TmemLayout(TmemLayoutKind.LANE_128, col_start=448))         # stage A: delta(G5)→NV(G6)
    shared_inp_b = k.tensor(space=MemorySpace.TMEM, dtype=iod, shape=(V_DIM, BT),
                            layout=TmemLayout(TmemLayoutKind.LANE_128, col_start=480))        # stage B: vnew_gated(G7)
    ...
    bar_spec = {
        ...
        "tq": (MBarKind.TMA, 1), "tv": (MBarKind.TMA, 1),
        "tg": (MBarKind.TMA, 1), "tb": (MBarKind.TMA, 1),
        "d_kk": (MBarKind.TCGEN05, 1), "d_qk": (MBarKind.TCGEN05, 1),
        "d_ks": (MBarKind.TCGEN05, 1), "d_qs": (MBarKind.TCGEN05, 1),
        "d_nv": (MBarKind.TCGEN05, 1), "d_oi": (MBarKind.TCGEN05, 1),
        "d_ds": (MBarKind.TCGEN05, 1),
        "gate_ready0": (MBarKind.THREAD, GATE_T), "gate_ready1": (MBarKind.THREAD, GATE_T),  # gate->CG0 / gate->CG1
        ...
        "f_kk": (MBarKind.THREAD, CG0_T), "f_qk": (MBarKind.THREAD, CG0_T),
        "f_ks": (MBarKind.THREAD, CG1_T), "f_qs": (MBarKind.THREAD, CG1_T),
        "f_nv": (MBarKind.THREAD, CG1_T), "f_oi": (MBarKind.THREAD, CG1_T),
        "chunk_free": (MBarKind.THREAD, CG1_T + 1),  # CG1 + epilogue (both free chunk-c buffers)
    }
    bars = {nm: k.mbar(kind=spec[0], stages=(spec[2] if len(spec) > 2 else 1))
    ...
    _emit(k, config, n_chunks, sched, task_geom,
          ...
          (r1, r2, r3, racc, rb16, rb16b, glast, frag, frag32, zrow, imm_a, imm_b, imm_acc, imm_accb, sttile, sttile2, sinp_reg),
          bars)
...
def _emit(k, config, n_chunks, sched, task_geom, args, sm, tm, rg, bars):
    ...
    r1, r2, r3, racc, rb16, rb16b, glast, frag, frag32, zrow, imm_a, imm_b, imm_acc, imm_accb, sttile, sttile2, sinp_reg = rg
    scale = config.scale
    ...
    def issue(dst, a, b, m, n, kk, done, accum0=False, trans_a=False, trans_b=False,
              a_row0=0, b_row0=0):
        ...
        for g in range(kk // 16):
            a_sl = (_sl(a, (a_row0 + g * 16, 0), (16, m)) if trans_a
                    else _sl(a, (a_row0, g * 16), (m, 16)))
            b_sl = (_sl(b, (b_row0 + g * 16, 0), (16, n)) if trans_b
                    else _sl(b, (b_row0, g * 16), (n, 16)))
            k.tcgen05_mma(dst, a_sl, b_sl,
                          m=m, n=n, k=16, accum=(accum0 or g != 0), trans_a=trans_a, trans_b=trans_b)
    ...
    def rss(dst_s, fac_neg_beta, mask_strict, tid, lane, warp, dst_bf16=False, acc=None):
        ...
        k.tcgen05_ld(frag, acc if acc is not None else acc_tmem, shape="16x256b", num=LD_NUM, row=0, col=0)
        k.tcgen05_wait_ld()
        for va in range(2):
    ...
    with k.role(warp=TMA_WARP):
        ...
        with k.for_each_task(sched) as task:   # tile loop (= CUTLASS PipelineState, never per-tile reset)
            ...
            with k.for_loop(stop=NCH) as c:   # runtime chunk loop (varlen num_chunks)
                ...
                k.tma_load(_sl(k_s, (kstg * BT, 0), (BT, K_DIM)), k_g, mbar=bars["tk"],
                           bytes=BT * K_DIM * 2, coords=(gtok, k_head, 0), mbar_stage=kstg,
                           shape=(BT, K_DIM), gmem_shape=(BT, 1, K_DIM))
                ...
                for (nm, dst, src, hd, cols) in (
                        ("tq", q_s, q_g, q_head, K_DIM), ("tv", v_s, v_g, v_head, V_DIM)):
                    k.mbarrier_arrive_expect_tx(bars[nm], bytes=BT * cols * 2)
                    k.tma_load(_sl(dst, (0, 0), (BT, cols)), src, mbar=bars[nm],
                               bytes=BT * cols * 2, coords=(gtok, hd, 0),
                               shape=(BT, cols), gmem_shape=(BT, 1, cols))
    ...
    with k.role(warp=GATE_WARP):
        ...
        with k.for_each_task(sched) as task:
            ...
            with k.for_loop(stop=NCH) as c:
                ...
                k.tma_load(gcs_s, gate_g, mbar=bars["tg"], bytes=BT * 4,
                           coords=(gtok, eh), shape=(BT,), gmem_shape=(BT, 1))
                k.mbarrier_arrive_expect_tx(bars["tb"], bytes=BT * 4)
                k.tma_load(beta_s, beta_g, mbar=bars["tb"], bytes=BT * 4,
                           coords=(gtok, eh), shape=(BT,), gmem_shape=(BT, 1))
                k.mbarrier_wait(bars["tg"], phase=ph(gc))
                if config.varlen:  # OOB tokens (global pos >= seqlen_b) -> gate=1 (log2=0, no decay)
                    for half in range(2):
                        i = lane + half * 32
                ...
                for step in range(6):
                    ...
                    k.warp_sync()
                k.mbarrier_wait(bars["tb"], phase=ph(gc))   # beta must be loaded before consumers read it
    ...
    with k.role(warp=MMA_WARP):
        ...
        with k.for_each_task(sched) as task:
            ...
            with k.for_loop(stop=NCH) as c:
                ...
                issue(acc_s0, k_s, k_s, BT, BT, K_DIM, "d_kk", a_row0=ks_row, b_row0=ks_row)  # GEMM1 W_kk -> stage 0
                k.mbarrier_wait(bars["f_kk"], phase=ph(gc))
                k.mbarrier_wait(bars["tq"], phase=ph(gc))
                issue(acc_s1, q_s, k_s, BT, BT, K_DIM, "d_qk", b_row0=ks_row)  # GEMM2 W_qk -> stage 1
                ...
                with k.if_(c > 0):   # chunk 0: S_prev=0, skip GEMM3/4 (flashinfer is_first_chunk)
                    ...
                    issue(acc_tmem, k_s, state_inp, BT, V_DIM, K_DIM, "d_ks", trans_b=True, a_row0=ks_row)  # GEMM3 K@S
                    k.mbarrier_wait(bars["f_ks"], phase=ph(gc_pos))
                    issue(qstate_tmem, q_s, state_inp, BT, V_DIM, K_DIM, "d_qs", trans_b=True)  # GEMM4 Q@S → q_state
                ...
                with k.if_(c > 0):
                    issue(acc_tmem, ainv_s, shared_inp, BT, V_DIM, BT, "d_nv")  # GEMM5 VNEW (delta from TMEM)
                ...
                issue(acc_tmem, attn_s, shared_inp, BT, V_DIM, BT, "d_oi")    # GEMM6 O_intra (NV from TMEM)
                k.mbarrier_wait(bars["f_oi"], phase=ph(gc))
                k.mbarrier_wait(bars["ktvng_ready"], phase=ph(gc))
                issue(s_tmem, k_s, shared_inp_b, K_DIM, V_DIM, BT, "d_ds", accum0=True, trans_a=True, a_row0=ks_row)  # GEMM7 dS
    ...
    with k.role(warpgroup=0):
        tid = k.tid_in_wg(); lane = tid % 32; warp = tid // 32
        gc = k.scalar(initial=0)   # cumulative chunk index (pipeline state across tiles)
        with k.for_each_task(sched) as task:
            seq, eh, q_head, k_head, v_head, tok_base, NCH, slen = task_geom(task)
            with k.for_loop(stop=NCH) as c:
                ...
                def _ldA(src, R, C):  # A = src[R:R+8,C:C+8] non-trans, m16 broadcast
                    k.ldmatrix(_sl(imm_a, (0,), (1,)), _sl(src, (R + lane % 8, C), (1, 8)),
                               num=1, trans=False)
                ...
                def _store8(dst, R, C, neg):
                    ...
                    k.stmatrix(_sl(dst, (R + lane % 8, C), (1, 8)),
                               _sl(imm_accb, (0,), (2,)), num=1, trans=False)
                        ...
                        for mi in range(tb):
                            for ni in range(tb):
                                ...
                                for ki in range(tb):
                                    ...
                                    k.mma_sync(imm_acc, imm_a, imm_b, imm_acc, m=16, n=8, k=8, ab_dtype=iod)
            ...
            with k.for_loop(stop=NCH) as c:
                ...
                with k.if_(c > 0):
                    ...
                    for half in range(2):
                        ...
                        k.tcgen05_wait_ld()
                        for cc in range(64):
                            k.reg_cvt(_sl(sinp_reg, (half * 64 + cc,), (1,)), _sl(frag32, (cc,), (1,)))
                            k.reg_mul(_sl(frag32, (cc,), (1,)), _sl(frag32, (cc,), (1,)), r1)
                        k.tcgen05_st(s_tmem, frag32, num=64, row=0, col=half * 64)  # decayed main state
                    k.tcgen05_st(state_inp, _sl(sinp_reg, (0,), (64,)), num=64, row=0, col=0)  # fp16 copy
                    k.tcgen05_wait_st()
                    ...
                    _read128_delta(k, acc_tmem, tmpt_s, v_s, gcs_s, frag, rb16, rb16b, sttile, r1, lane, warp)
                    k.wg_sync(barrier_id=11)
                    k.mbarrier_arrive(bars["f_ks"])
                    k.mbarrier_wait(bars["d_qs"], phase=ph(gc_pos))
                    _read128_ointer(k, qstate_tmem, out_s, gcs_s, scale, frag, r1, sttile, lane, warp)
                    k.wg_sync(barrier_id=11)
                    ...
                    with k.if_(tid < V_DIM):
                        ...
                        k.tcgen05_st(shared_inp, _sl(sinp_reg, (0,), (BT // 2,)), num=BT // 2, row=0, col=0)
                    k.tcgen05_wait_st()
                    k.wg_sync(barrier_id=11)
                with k.if_(c.eq(0)):
                ...
                _read128_vnew(k, acc_tmem, vnewt_s, tmpt_s, gcs_s, glast, frag, sttile, sttile2, r1, lane, warp)
                k.wg_sync(barrier_id=11)
                ...
                with k.if_(tid < V_DIM):
                    ...
                    for j in range(BT):
                        k.reg_load(_sl(sinp_reg, (j,), (1,)), _sl(tmpt_s, (tid, j), (1, 1)))
                    k.tcgen05_st(shared_inp_b, _sl(sinp_reg, (0,), (BT // 2,)), num=BT // 2, row=0, col=0)
                k.tcgen05_wait_st()
                k.wg_sync(barrier_id=11)
                fence_pub(11)
        ...
        with k.role(warp=EPI_WARP, elected=True):
            ...
            with k.for_each_task(sched) as task:
                ...
                with k.for_loop(stop=NCH) as c:   # runtime chunk loop
                    ...
                    gtok = tok_base + c * BT
                    k.tma_store(out_g, _sl(out_s, (0, 0), (BT, V_DIM)),
                                coords=(gtok, eh, 0), shape=(BT, V_DIM), gmem_shape=(BT, 1, V_DIM))

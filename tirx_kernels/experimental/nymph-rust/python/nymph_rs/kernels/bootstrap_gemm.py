...
def build_bootstrap_gemm(M=256, N=128, K=64, dtype=DType.F16):
    ...
    with k.kernel_init(warp=0):
        k.tmem_alloc(tmem_base, n_cols=N, cta_group=CTA_GROUP)
    ...
    with k.role(warp=4):  # loader
        ...
        k.tma_load(a_s, a_g, mbar=smem_full, bytes=a_tb, coords=(a_m, 0), shape=(BLK_M, K))
        k.tma_load(b_s, b_g, mbar=smem_full, bytes=b_tb, coords=(b_n, 0), shape=(BLK_N, K))
    ...
    with k.role(warpgroup=0):  # epilogue
        ...
        for cb in range(N // 8):
            col = cb * 8
            k.tcgen05_ld(accum_frag, accum, num=8, row=0, col=col)
            k.tcgen05_wait_ld()
    ...
    with k.kernel_finalize(warp=0):
        k.tmem_dealloc(tmem_base, n_cols=N, cta_group=CTA_GROUP)
    return k.build()

"""Value + protocol tests for the chunked Gated Delta Net prefill kernel.

The kernel is the instruction-faithful nymph port of the FlashInfer Blackwell
CuteDSL chunked GDN prefill (see ``docs/kernels/gdn_prefill/INSTR.md`` for the per-call-site
CuteDSL→instruction mapping). Values are checked against the validated numpy
...
import _gdn_oracle as oracle  # noqa: E402
...
def test_gdn_prefill_heads(hc):
    ...
    res = nr.interpret(kernel, {q_g: bf(q), k_g: bf(k), v_g: bf(v),
                                gate_g: np.exp(g).astype(np.float32), beta_g: beta})
    ...
    for eh in range(neff):
        ...
        o_ref, s_ref = oracle.chunked(q[:, qh], k[:, kh], v[:, vh],
                                      g[:, eh], beta[:, eh], cfg.scale, None, BT=64)
...
def test_gdn_prefill_varlen(cfg):
    ...
    NS = len(seqlens)
    maxlen = max(seqlens)
    cu = np.array([0] + list(np.cumsum(seqlens)), np.int32)
    ...
    bf = lambda x: x.astype(ml_dtypes.bfloat16)  # noqa: E731
    inputs = {
        q_g: bf(q), k_g: bf(k), v_g: bf(v),
        gate_g: np.exp(g).astype(np.float32), beta_g: beta, cu_g: cu,
    }
    ...
    for s in range(NS):
        ...
        for hv in range(HV):
            hqk = hv // GVA_RATIO
            o_ref, s_ref = oracle.chunked(q[a:b, hqk, :], k[a:b, hqk, :], v[a:b, hv, :],
...
def test_gdn_prefill_value(cfg, io_dtype):
    ...
    res = nr.interpret(kernel, {
        q_g: bf(q), k_g: bf(k), v_g: bf(v), gate_g: raw_gate, beta_g: beta,
    })
    out = np.asarray(res[out_g.id], dtype=np.float32)
    ...
    for s in range(num_seqs):
        ...
        for hv in range(HV):
            hqk = hv // GVA_RATIO
            o_ref, s_ref = oracle.chunked(q[sl, hqk, :], k[sl, hqk, :], v[sl, hv, :],

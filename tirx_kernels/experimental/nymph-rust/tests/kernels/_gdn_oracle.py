...
algorithm). Cross-validated against each other. Pure numpy (no torch) so it can
back the nymph value-sim test."""
import numpy as np
...
def chunked(q, k, v, g, beta, scale, S0=None, BT=64):
    ...
    pad = (BT - T % BT) % BT
    if pad:
        q=np.pad(q,((0,pad),(0,0))); k=np.pad(k,((0,pad),(0,0))); v=np.pad(v,((0,pad),(0,0)))
        g=np.pad(g,(0,pad)); beta=np.pad(beta,(0,pad))
    q=q.astype(np.float64)*scale; k=k.astype(np.float64); v=v.astype(np.float64)
    ...
    for i in range(n):
        ...
        aqk = (q[i] @ k[i].T) * Lm; aqk[tri1] = 0.0    # intra attn (causal incl diag)
        v_prime = kcd @ S
        v_new = nv - v_prime
...
for trial in range(8):
    ...
    T = max(8, T)
    K=V=128
    q=rng.standard_normal((T,K))*0.3; k=rng.standard_normal((T,K))*0.3; v=rng.standard_normal((T,V))*0.3
    ...
    eo=np.abs(o1-o2).max(); es=np.abs(s1-s2).max()
    maxerr_o=max(maxerr_o,eo); maxerr_s=max(maxerr_s,es)
    print(f"  T={T:4d} S0={'y' if S0 is not None else 'n'}: max|Δo|={eo:.2e} max|ΔS|={es:.2e}")
print(f"OVERALL: max|Δo|={maxerr_o:.2e}  max|ΔS|={maxerr_s:.2e}  -> {'AGREE' if max(maxerr_o,maxerr_s)<1e-9 else 'MISMATCH'}")

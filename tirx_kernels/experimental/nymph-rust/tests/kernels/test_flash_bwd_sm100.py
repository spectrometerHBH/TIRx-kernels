...
Runs the kernel under the PER-WARP execution model: every protocol assertion
below exercises the per-thread mbarrier counts / single-thread issue elections
/ publish syncs of the migrated kernel (see flash_bwd_sm100.py's bar_spec).
...
def _run_odd_nnb(B, Hq, Hkv, Sq, Sk, Dq, Dv):
    ...
    pre-scaled in-epilogue (no xscale); GQA dk_g is the unscaled f32 reduce accumulator (xscale);
    dv_g direct; dq_g is the unscaled dQaccum (xscale)."""
    G = Hq // Hkv
    ...
    kernel = build_flash_bwd_sm100(cfg)
    qg, kg, vg, dog, lseg, dpsg, dqg, dkg, dvg = kernel.args
    dks = scale if G > 1 else 1.0  # GQA dKaccum unscaled (xscale); MHA dK pre-scaled in epilogue
    return (
        kernel,
        scale,
        dict(
...
# GQA x 2-CTA multi-Q-tile (n_mb=2). dK/dV f32 reduce-add over the q-heads of a group; the
...
# GQA x hd192 multi-Q-tile (n_mb>1): the dK/dV f32 group reduce-add interacts with the single-stage
# time-mux + dedicated-overlap WARs across mb. dQ/dK xscale (dQaccum / GQA-unscaled dK); dV direct.
GQA_HD192_NMB = [
    {
        "num_head": 4,
        "num_head_kv": 2,
...
def test_flash_bwd_sm100_gqa_hd192_nmb(cfg):
    # GQA x hd192 multi-Q-tile: trace + protocol Passed (commutative-reduce) + dQ/dK/dV value.
...
#   - LARGE_PROTO (n_nb==1, or single-cluster 2-CTA, or batchxheads): protocol Passed + value.
...
def _assert_big_value(kernel, scale, info):
    res = nr.interpret(kernel, info["inputs"])
    # dQ output is the unscaled dQaccum (xscale to compare); dK/dV are direct.
...
# ===================== CROSS-PRODUCT: causal x head_dim (>hd64) =====================
...
def test_flash_bwd_sm100_gqa_hd96():
    ...
    # the ld-split. dK is the unscaled fp32 reduce accumulator -> xscale; dV unscaled; dQ xscale.
    kernel, scale, info = _run_gqa(
...
# deterministic x 2-CTA (hd128/hd192): the dQ GMEM semaphore serializes the cross-CLUSTER
...
LARGE_PROTO = [
    ...
    ("2cta_hd128_b2h4_sq128sk256", 2, 4, 128, 128, 128, 256),  # single cluster x 8 head-batch
...
#      exercises ALL THREE tile classes: kv0xq0 diagonal (partial), kv0xq1 fully-below (no
#      mask), kv1xq0 fully-above (masked to all-zero P), kv1xq1 diagonal. VALUE-ONLY: with
...
def _run_varlen(cfg_d, D=64, causal=False):
    ...
    NS = len(seqlens)
    maxlen = max(seqlens)
    cu = np.array([0, *np.cumsum(seqlens)], np.int32)  # cu[b]=start, cu[b+1]-cu[b]=len
...
# ===================== VARLEN x 2-CTA (head_dim=128 -> cluster_size=2) =====================
...
# GQA x 2-CTA (head_dim>=128 -> cluster_size=2). The dK/dV epilogue stages f32 into sdK/sdV
...
def test_flash_bwd_sm100_gqa_2cta(cfg):
    # GQA x 2-CTA: trace + check_protocol Passed (commutative-reduce) + dQ/dK/dV value.
...
# per-feature tests above exercise in isolation — causalx2ctaxmulti-Q-tile, GQAxcausal, detxGQA,
# detx2cta, hd192x{causal,gqa,det}xmulti-Q-tile, varlen handled separately. Locks in the matrix.
CROSS_CONFIG = [

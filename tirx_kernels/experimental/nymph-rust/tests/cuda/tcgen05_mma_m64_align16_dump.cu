// Hand-written tcgen05.mma harness: M=64, N=128, K=16, cta_group=1, kind::f16,
// Layout F with d-tmem lane field = 16 (the upper-half / align-16 placement).
// Identical to tcgen05_mma_m64_dump.cu except the MMA [d-tmem] operand carries
// a lane field of 16, so the 64 result rows land in the HIGH 16 of each 32-lane
// subpartition (lanes {16-31, 48-63, 80-95, 112-127}) instead of the low 16.
// The readback still scans all 128 lanes, so the dump captures the shifted
// rows.
//
...
__global__ void mma_kernel(const __half *gA, const __half *gB, float *out) {
  ...
  if (tid == 0) {
    ...
    // d-tmem lane field = 16: place the 64 rows in the high 16 of each
    // subpartition
    asm volatile("{\n\t.reg .pred p;\n\t"
                 "setp.ne.b32 p, %4, 0;\n\t"
  ...
  // 32x32b.x8 loops (scans all 128 lanes regardless of where the rows landed)

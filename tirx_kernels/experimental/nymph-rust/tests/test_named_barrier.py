...
DIFFERENT roles on a shared barrier_id and establishes a cross-stream happens-before
edge in the protocol checker (bar.sync's release/acquire semantics).
"""
...
from nymph_rs import IRBuilder, MemorySpace, DType, TensorSlice
...
def _build(with_barrier: bool):
    ...
    # SMEM write->read is ordered ONLY by the named barrier between the roles.

...
Standalone proofs (isolated from the full flash-bwd kernel):
  ...
  NEGATIVE A — the SAME two reduce-adds WITHOUT the wrap → Passed (commutative
               atomic) but a nondeterministic_reduction warning (the sem edge is
               load-bearing for reproducibility, not a coincidence).
  NEGATIVE B — (b1) wrong turn (each CTA waits an unreachable value) → the awaited
               value is never signalled → DEADLOCK (caught, not silently Passed);
               (b2) both wait 0 (no serialization) and (b3) signal-BEFORE-drain
               (atomic_add before the wait_group drain) → the release does not
               order the reduce-adds → the warning survives.

Per-warp adaptation of the nymph-codegen original: `role(warpgroup=0)` becomes
`if_warpgroup(0)`; the elected single-thread branches (`tid_in_wg()==0`) already
satisfy the per-warp single-thread issue gates on tma_reduce_add/gmem_atomic_add.
"""
...
def _build(
    *,
    ...
    same_turn           — both CTAs wait 0 → both pass immediately (no serialization);
                          the checker must still warn (the edge is not real).

...
| `TmaLoad` | `Tx.copy_async(smem[slice], gmem[slice], dispatch="tma", cta_group=…, mbar=…, prefetch_tensormap=True)` | the per-CTA A-row / B-band split is in the IR's coords; codegen prints them. The transfer size is derived from the tile extents (`numel(shape) × dtype`) — the IR carries NO `bytes` field, so sim tx-accounting and TIRx read the same fact in one place |

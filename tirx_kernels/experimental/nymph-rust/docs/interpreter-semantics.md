...
| `SetMaxNReg` | Reviewed metadata | `setmaxnreg` directive for the enclosing warp(s); simulation metadata only, register pressure is not modeled. | Validation requires a statically-resolvable branch covering whole warp(s) and `nreg` a positive multiple of 8. |

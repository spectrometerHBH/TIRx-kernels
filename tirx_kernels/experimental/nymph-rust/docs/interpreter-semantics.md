...
| `ScalarDef`, `ScalarStore`, `ScalarLet`, `StoreScalar` | Reviewed scalar | Per-thread scalar values in the active cohort. `ScalarDef(initial=TensorSlice)` can load one GMEM/SMEM scalar in value mode, and in trace mode when the scalar cell is concrete and valid. `ScalarLet` binds its value like a store but is single-assignment (a validate-time rule; no runtime difference). `StoreScalar` writes a scalar expression to an SMEM scalar slice in both modes. | Missing input in value mode, invalid skipped-payload scalar bridge in trace mode, unsupported scalar dtype, unresolved expression, undefined variable. |

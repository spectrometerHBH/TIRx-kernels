...
class Barriers:
    ...
    def init(self, threads_num_wait):
        ...
        if self.pipe_depth == 1:
            if tid == 0:
                T.ptx.mbarrier.init.shared.b64(self.mbar.ptr_to([0]), T.uint32(threads_num_wait))
        elif tid == 0:
            for i in T.serial(self.pipe_depth):
                T.ptx.mbarrier.init.shared.b64(self.mbar.ptr_to([i]), T.uint32(threads_num_wait))
...
class SmemManager:
    ...
    def init(self):
        ...
        if tid == 0:
            for i in T.serial(self.chunk_num):
                T.ptx.mbarrier.init.shared.b64(self.mbar.ptr_to([i]), T.uint32(1))
    ...
    def arrive_all(self, level: Literal["cta", "warpgroup"] = "cta"):
        ...
        if level == "cta":
            ...
            if warp_id == 0:
                if lane_id < self.chunk_num:
                    T.ptx.mbarrier.arrive.shared.b64(self.mbar.ptr_to([lane_id]), T.uint32(1))
        elif level == "warpgroup":
            self.reg_count[0] = 0
            T.ptx.bar.sync(T.uint32(6 + wg_id), 128)
            if warp_id % KernelConfig.WARP_NUMBER == 0:
    ...
    def arrive_chunk(self, chunk_id):
        T.ptx.mbarrier.arrive.shared.b64(self.mbar.ptr_to([chunk_id]), T.uint32(1))

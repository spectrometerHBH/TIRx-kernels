...
def test_sparse_flashmla_prefill_bench_configs_cover_three_impls() -> None:
    selected = {select_kernel(**cfg) for cfg in BENCH_CONFIGS}
    assert selected == {
        "sparse_flashmla_prefill_head64_phase1",
        "sparse_flashmla_prefill_head128_phase1",
        "sparse_flashmla_prefill_head128_small_topk_phase1",
    }
    labels = [cfg["label"] for cfg in BENCH_CONFIGS]
